...
class Transform():
    ...
    def transform(self, raw_data): # needs test
        ...
        for row in raw_data: # row: ['2020-06-11 13:26:25.441651', 'Uppingham', 'James Williams', ' Glass of milk: 0.7', 0.7, 'CARD', 'Type: mastercard\nCCN: 5439844399567818']
            ...
            try:
                ...
                t_date, t_time = self.date_breaker(row[0])  # defines variables for split date and time from date breaker
            location_id = self.get_id(row[1])
            print(f"time is {t_time}")
            print(f"date is {t_date}")

            print(f" time type is {type(t_time)}")
            print(f" date type is {type(t_date)}")

            trans_id = self.id_maker(str(t_date), str(t_time),location_id)
    ...
    def id_maker(self, date_str, time_str, location_id):
        characters_to_remove = "- :"
        dot_position = time_str.find(".")
        chopped_time_str = time_str[:dot_position]
        new_string = date_str + "." + chopped_time_str
    ...
    def location_adder(self, location, location_list): # tested
    ...
    def drink_flav_breaker(self, drink): # needs testing
        broken_flavour = []
        flavour = None
        if " - " in drink:
    ...
    def order_to_drink_ids(self, raw_orders): # needs testing
    ...
    def get_id(self, split_item): # functions for drink OR location Tested
    ...
    def item_adder(self, split_item, item_dict): #tested
        max_id = max(item_dict.values())
        new_id = max_id + 1
        return(new_id)
        
    def get_split_drink(self, raw_drink): #tested
    ...
    def get_price_and_flavour(self, drink_string): #tested
    ...
    def drink_name_size_breaker(self, drink): # tested
        name_broken = []
        drink_size = "N/A"
        if "Large" in drink:
            drink_size = "Large"
    ...
    def date_breaker(self, date): # tested
        split_date = date.date()
        split_time = date.time()
        return (split_date, split_time) 

    def csv_date_breaker(self, date): # tested
        #  date is 3075814
